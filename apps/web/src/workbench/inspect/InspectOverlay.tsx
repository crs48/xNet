 ...
 * Release the key and it is gone.
 *
 ...
 *
 * This is W1 and it deliberately CANNOT EDIT ANYTHING. Shipping the resolver
 * before the editor is the point: if the lane it names is wrong, we find out
 * while the consequence is a misleading label rather than a wrong commit.
 */
import { resolveLane, type Resolution } from '@xnetjs/devkit/blast-radius'
import { useCallback, useEffect, useRef, useState } from 'react'
import { getSlotView } from '../slot-registry'
import { buildTokenIndex, resolvePointed, tokenRefFor } from './resolve-pointed'
...
export function InspectOverlay(): React.JSX.Element | null {
  ...
  const setHeld = useCallback(
    (held: boolean) => {
      ...
      tokenIndex.current = buildTokenIndex(getComputedStyle(document.documentElement))
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => setHeld(event.altKey)
