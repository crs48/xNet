var Y=["interactive","bulk","write"];function I(){return typeof performance<"u"&&typeof performance.now=="function"?performance.now():Date.now()}var H=class{constructor(e){this.onOp=e}queues={interactive:[],bulk:[],write:[]};coalesced=new Map;running=!1;schedule(e,t,a,n){if(a!==void 0){const s=this.coalesced.get(a);if(s)return s}const r=new Promise((s,i)=>{this.queues[e].push({run:t,resolve:s,reject:i,lane:e,label:n,enqueuedAt:I()})});if(a!==void 0){this.coalesced.set(a,r);const s=()=>{this.coalesced.get(a)===r&&this.coalesced.delete(a)};r.then(s,s)}return this.pump(),r}snapshot(){return{interactive:this.queues.interactive.length,bulk:this.queues.bulk.length,write:this.queues.write.length,inFlight:this.running}}next(){for(const e of Y){const t=this.queues[e].shift();if(t)return t}}async pump(){if(!this.running){this.running=!0;try{let e;for(;e=this.next();){const t=I();try{e.resolve(await e.run())}catch(a){e.reject(a)}finally{if(this.onOp){const a=I();this.onOp({lane:e.lane,label:e.label,queueMs:t-e.enqueuedAt,execMs:a-t})}}}}finally{this.running=!1}}}};function C(e){return globalThis}function z(e){return typeof C().navigator?.storage?.getDirectory=="function"}function Q(e){const t=C();if(typeof t.FileSystemSyncAccessHandle<"u")return!0;const a=t.FileSystemFileHandle?.prototype;return!!(a&&"createSyncAccessHandle"in a)}function V(e){const t=C();return typeof t.SharedArrayBuffer<"u"&&t.crossOriginIsolated===!0}function j(e){const t=z(),a=Q(),n=V();let r,s;return t?a?(r="sync-access-handle",s="OPFS sync access handles available — using the durable opfs-sahpool fast path."):(r="async-opfs",s="OPFS is available but sync access handles are not (iOS 15.2–16.3 or an older WebView); falling back to the slower async OPFS backend. Data still persists."):(r="memory",s="OPFS is unavailable in this context (private browsing or an unsupported engine); local data will not persist across reloads."),{opfs:t,syncAccessHandle:a,crossOriginIsolated:n,mode:r,reason:s}}var K=11,$=26;function J(e){return!e||typeof e!="object"?null:e}function R(e){return v(e,new Set)}function v(e,t){const a=J(e);if(!a||t.has(e))return!1;t.add(e);const n=typeof a.message=="string"?a.message.toLowerCase():String(a.message),r=typeof a.code=="string"?a.code.toUpperCase():"",s=typeof a.resultCode=="number"?a.resultCode:null;return s===K||s===$||r==="SQLITE_CORRUPT"||r==="SQLITE_NOTADB"||n.includes("sqlite_corrupt")||n.includes("sqlite_notadb")||n.includes("database disk image is malformed")||n.includes("file is not a database")||v(a.cause,t)}var Z=7,ee=`
-- ============================================
...
`,ae=ee+te;function U(e){const t=e;return t?t.name==="NoModificationAllowedError"?!0:/Access Handles cannot be created|createSyncAccessHandle/i.test(t.message??""):!1}async function re(e,t={}){const a=t.attempts??5,n=t.baseDelayMs??150,r=t.sleep??(i=>new Promise(o=>setTimeout(o,i)));let s;for(let i=1;i<=a;i++)try{return await e()}catch(o){if(s=o,!U(o)||i===a)throw o;t.onRetry?.(i,o),await r(n*i)}throw s}var F="opfs-sahpool",g=".xnet-sqlite",M=10;function se(){return typeof self<"u"&&typeof localStorage<"u"&&localStorage.getItem("xnet:sqlite:debug")==="true"}function c(...e){se()&&console.log(...e)}function b(e){return e.path.startsWith("/")?e.path:`/${e.path}`}async function ne(e){const t=globalThis.navigator?.storage;if(typeof t?.getDirectory=="function")try{await(await t.getDirectory()).removeEntry(e.replace(/^\/+/,""),{recursive:!0})}catch(a){if(a instanceof DOMException&&a.name==="NotFoundError")return;throw a}}async function ie(e){const t=(await import("./index-BBe9ZyCD.js")).default,a=await t(),n=b(e);try{const r=await a.installOpfsSAHPoolVfs({name:F,directory:g,initialCapacity:M,clearOnInit:!1});try{r.unlink(n)}catch{}await r.wipeFiles(),await r.removeVfs()}catch(r){console.warn("[WebSQLiteAdapter] SAH-pool reset failed, removing OPFS directory:",r),await ne(g)}}var oe=class{sqlite3=null;db=null;poolUtil=null;_config=null;inTransaction=!1;storageMode="memory";async open(e){if(this.db!==null)throw new Error("Database already open. Call close() first.");c("[WebSQLiteAdapter] Starting open()..."),c("[WebSQLiteAdapter] Importing sqlite-wasm...");const t=(await import("./index-BBe9ZyCD.js")).default;c("[WebSQLiteAdapter] sqlite-wasm imported"),c("[WebSQLiteAdapter] Initializing sqlite3 module..."),this.sqlite3=await t(),c("[WebSQLiteAdapter] sqlite3 module initialized");try{await re(async()=>{c("[WebSQLiteAdapter] Installing OPFS-SAHPool VFS..."),this.poolUtil=await this.sqlite3.installOpfsSAHPoolVfs({name:F,directory:g,initialCapacity:M,clearOnInit:!1}),c("[WebSQLiteAdapter] OPFS-SAHPool VFS installed"),c("[WebSQLiteAdapter] Reserving capacity..."),await this.poolUtil.reserveMinimumCapacity(10),c("[WebSQLiteAdapter] Capacity reserved");const a=b(e);c("[WebSQLiteAdapter] Opening database at",a),this.db=new this.poolUtil.OpfsSAHPoolDb(a,"c"),this.storageMode="opfs",c("[WebSQLiteAdapter] Database opened with OPFS-SAHPool")},{onRetry:(a,n)=>{console.warn(`[WebSQLiteAdapter] OPFS access handles are busy (attempt ${a}) — a previous tab/worker is likely still releasing them; retrying before any in-memory fallback.`,n)}})}catch(a){const n=j();n.mode==="async-opfs"?console.info("[WebSQLiteAdapter] Sync access handles unavailable — "+n.reason):console.warn("[WebSQLiteAdapter] OPFS-SAHPool not available, trying OPFS direct mode:",a);const r=b(e),s=this.sqlite3?.oo1?.OpfsDb;if(typeof s=="function")try{this.db=new s(r,"c"),this.storageMode="opfs",c("[WebSQLiteAdapter] Database opened with OPFS direct mode")}catch(i){c("[WebSQLiteAdapter] OPFS direct mode not available:",i),this.db=new this.sqlite3.oo1.DB(":memory:","c"),this.storageMode="memory"}else this.db=new this.sqlite3.oo1.DB(":memory:","c"),this.storageMode="memory";if(this.storageMode==="memory"){const i=U(a)?"another xNet tab/worker is holding the local database":"OPFS is unavailable in this browser context";console.error(`[WebSQLiteAdapter] Using an IN-MEMORY database (${i}). Local data will NOT persist across reloads and the workspace will appear empty until it re-syncs from the hub. Close other xNet tabs and reload to restore persistent storage.`)}}this._config=e,e.foreignKeys!==!1&&this.execSync("PRAGMA foreign_keys = ON"),e.busyTimeout?this.execSync(`PRAGMA busy_timeout = ${e.busyTimeout}`):this.execSync("PRAGMA busy_timeout = 5000");try{this.execSync("PRAGMA page_size = 8192")}catch(a){c("[WebSQLiteAdapter] page_size pragma not applied:",a)}this.execSync("PRAGMA synchronous = NORMAL"),this.execSync("PRAGMA cache_size = -262144");try{this.execSync("PRAGMA mmap_size = 268435456")}catch(a){c("[WebSQLiteAdapter] mmap_size pragma not applied:",a)}this.execSync("PRAGMA temp_store = MEMORY");try{this.execSync("PRAGMA journal_mode = TRUNCATE")}catch(a){c("[WebSQLiteAdapter] journal_mode pragma not applied:",a)}}async close(){if(this.db){try{this.execSync("PRAGMA optimize")}catch(e){c("[WebSQLiteAdapter] optimize on close skipped:",e)}this.db.close(),this.db=null}this.sqlite3=null,this.poolUtil=null,this._config=null}isOpen(){return this.db!==null}getStorageMode(){return this.storageMode}async query(e,t){this.ensureOpen();const a=[];return this.db.exec({sql:e,bind:t,rowMode:"object",callback:n=>{a.push(n)}}),a}async queryOne(e,t){return(await this.query(e,t))[0]??null}async run(e,t){return this.ensureOpen(),this.db.exec({sql:e,bind:t}),{changes:this.sqlite3.capi.sqlite3_changes(this.db.pointer),lastInsertRowid:this.sqlite3.capi.sqlite3_last_insert_rowid(this.db.pointer)}}async exec(e){this.ensureOpen(),this.execSync(e)}execSync(e){this.db.exec({sql:e})}async transaction(e){await this.beginTransaction();try{const t=await e();return await this.commit(),t}catch(t){if(this.inTransaction)try{await this.rollback()}catch(a){if(R(a)&&!R(t))throw a}throw t}}async applyNodeBatch(e){return await this.transaction(async()=>{for(const t of e.nodes)await this.run(`INSERT INTO nodes (id, schema_id, created_at, updated_at, created_by, deleted_at)
         ...
         ON CONFLICT(key) DO UPDATE SET value = excluded.value`,[String(e.lastLamportTime)]),e.indexMode!=="defer-schema"){const t=Date.now();for(const a of e.affectedSchemaIds)await this.run(`UPDATE node_query_materializations
             SET invalidated_at = ?
             WHERE schema_id = ? AND invalidated_at IS NULL`,[t,a])}}),{nodeRowsWritten:e.nodes.length,propertyRowsWritten:e.properties.length,changeRowsWritten:e.changes.length,scalarRowsWritten:e.scalarIndexRows.length,ftsRowsWritten:e.ftsRows.length}}async beginTransaction(){if(this.inTransaction)throw new Error("Transaction already in progress");this.execSync("BEGIN IMMEDIATE"),this.inTransaction=!0}async commit(){if(!this.inTransaction)throw new Error("No transaction in progress");try{this.execSync("COMMIT"),this.inTransaction=!1}catch(e){throw R(e)&&(this.inTransaction=!1),e}}async rollback(){if(this.inTransaction)try{this.execSync("ROLLBACK")}finally{this.inTransaction=!1}}async prepare(e){return{query:async t=>this.query(e,t),queryOne:async t=>this.queryOne(e,t),run:async t=>this.run(e,t),finalize:async()=>{}}}async getSchemaVersion(){try{return(await this.queryOne("SELECT version FROM _schema_version ORDER BY version DESC LIMIT 1"))?.version??0}catch{return 0}}async setSchemaVersion(e){await this.run("INSERT INTO _schema_version (version, applied_at) VALUES (?, ?)",[e,Date.now()])}async applySchema(e,t){return await this.getSchemaVersion()>=e?!1:(await this.transaction(async()=>{await this.exec(t),await this.setSchemaVersion(e)}),!0)}async getDatabaseSize(){try{return(await this.queryOne("SELECT page_count * page_size as size FROM pragma_page_count(), pragma_page_size()"))?.size??0}catch{return 0}}async vacuum(){await this.exec("VACUUM")}async checkpoint(){return 0}ensureOpen(){if(!this.db||!this.sqlite3)throw new Error("Database not open. Call open() first.")}};async function de(e){const t=new oe;return await t.open(e),await t.applySchema(Z,ae),t}var ce="__xnetSqliteBootLog";function le(e){return{[ce]:e}}/**
 * @license
 * Copyright 2019 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */const P=Symbol("Comlink.proxy"),Ee=Symbol("Comlink.endpoint"),ue=Symbol("Comlink.releaseProxy"),A=Symbol("Comlink.finalizer"),S=Symbol("Comlink.thrown"),k=e=>typeof e=="object"&&e!==null||typeof e=="function",pe={canHandle:e=>k(e)&&e[P],serialize(e){const{port1:t,port2:a}=new MessageChannel;return L(e,t),[a,[a]]},deserialize(e){return e.start(),Ne(e)}},Te={canHandle:e=>k(e)&&S in e,serialize({value:e}){let t;return e instanceof Error?t={isError:!0,value:{message:e.message,name:e.name,stack:e.stack}}:t={isError:!1,value:e},[t,[]]},deserialize(e){throw e.isError?Object.assign(new Error(e.value.message),e.value):e.value}},q=new Map([["proxy",pe],["throw",Te]]);function he(e,t){for(const a of e)if(t===a||a==="*"||a instanceof RegExp&&a.test(t))return!0;return!1}function L(e,t=globalThis,a=["*"]){t.addEventListener("message",function n(r){if(!r||!r.data)return;if(!he(a,r.origin)){console.warn(`Invalid origin '${r.origin}' for comlink proxy`);return}const{id:s,type:i,path:o}=Object.assign({path:[]},r.data),u=(r.data.argumentList||[]).map(T);let d;try{const l=o.slice(0,-1).reduce((E,N)=>E[N],e),p=o.reduce((E,N)=>E[N],e);switch(i){case"GET":d=p;break;case"SET":l[o.slice(-1)[0]]=T(r.data.value),d=!0;break;case"APPLY":d=p.apply(l,u);break;case"CONSTRUCT":{const E=new p(...u);d=me(E)}break;case"ENDPOINT":{const{port1:E,port2:N}=new MessageChannel;L(e,N),d=Oe(E,[E])}break;case"RELEASE":d=void 0;break;default:return}}catch(l){d={value:l,[S]:0}}Promise.resolve(d).catch(l=>({value:l,[S]:0})).then(l=>{const[p,E]=m(l);t.postMessage(Object.assign(Object.assign({},p),{id:s}),E),i==="RELEASE"&&(t.removeEventListener("message",n),W(t),A in e&&typeof e[A]=="function"&&e[A]())}).catch(l=>{const[p,E]=m({value:new TypeError("Unserializable return value"),[S]:0});t.postMessage(Object.assign(Object.assign({},p),{id:s}),E)})}),t.start&&t.start()}function _e(e){return e.constructor.name==="MessagePort"}function W(e){_e(e)&&e.close()}function Ne(e,t){const a=new Map;return e.addEventListener("message",function(r){const{data:s}=r;if(!s||!s.id)return;const i=a.get(s.id);if(i)try{i(s)}finally{a.delete(s.id)}}),w(e,a,[],t)}function y(e){if(e)throw new Error("Proxy has been released and is not useable")}function G(e){return h(e,new Map,{type:"RELEASE"}).then(()=>{W(e)})}const f=new WeakMap,O="FinalizationRegistry"in globalThis&&new FinalizationRegistry(e=>{const t=(f.get(e)||0)-1;f.set(e,t),t===0&&G(e)});function ye(e,t){const a=(f.get(t)||0)+1;f.set(t,a),O&&O.register(e,t,e)}function Se(e){O&&O.unregister(e)}function w(e,t,a=[],n=function(){}){let r=!1;const s=new Proxy(n,{get(i,o){if(y(r),o===ue)return()=>{Se(s),G(e),t.clear(),r=!0};if(o==="then"){if(a.length===0)return{then:()=>s};const u=h(e,t,{type:"GET",path:a.map(d=>d.toString())}).then(T);return u.then.bind(u)}return w(e,t,[...a,o])},set(i,o,u){y(r);const[d,l]=m(u);return h(e,t,{type:"SET",path:[...a,o].map(p=>p.toString()),value:d},l).then(T)},apply(i,o,u){y(r);const d=a[a.length-1];if(d===Ee)return h(e,t,{type:"ENDPOINT"}).then(T);if(d==="bind")return w(e,t,a.slice(0,-1));const[l,p]=D(u);return h(e,t,{type:"APPLY",path:a.map(E=>E.toString()),argumentList:l},p).then(T)},construct(i,o){y(r);const[u,d]=D(o);return h(e,t,{type:"CONSTRUCT",path:a.map(l=>l.toString()),argumentList:u},d).then(T)}});return ye(s,e),s}function fe(e){return Array.prototype.concat.apply([],e)}function D(e){const t=e.map(m);return[t.map(a=>a[0]),fe(t.map(a=>a[1]))]}const B=new WeakMap;function Oe(e,t){return B.set(e,t),e}function me(e){return Object.assign(e,{[P]:!0})}function m(e){for(const[t,a]of q)if(a.canHandle(e)){const[n,r]=a.serialize(e);return[{type:"HANDLER",name:t,value:n},r]}return[{type:"RAW",value:e},B.get(e)||[]]}function T(e){switch(e.type){case"HANDLER":return q.get(e.name).deserialize(e.value);case"RAW":return e.value}}function h(e,t,a,n){return new Promise(r=>{const s=Le();t.set(s,r),e.start&&e.start(),e.postMessage(Object.assign({id:s},a),n)})}function Le(){return new Array(4).fill(0).map(()=>Math.floor(Math.random()*Number.MAX_SAFE_INTEGER).toString(16)).join("-")}function X(e,t,a){return`${e}\0${t}\0${a?JSON.stringify(a):""}`}function x(...e){console.info(...e);try{self.postMessage(le(e))}catch{}}function Ie(){return typeof self<"u"&&typeof localStorage<"u"&&localStorage.getItem("xnet:sqlite:debug")==="true"}function _(...e){Ie()&&console.log(...e)}_("[SQLiteWorker] Worker script loaded, Comlink imported");var Re=class{adapter=null;bootDebug=!1;scheduler=new H(e=>{this.bootDebug&&x("[xNet] sqlite op",e.label??e.lane,{lane:e.lane,queueMs:Math.round(e.queueMs),execMs:Math.round(e.execMs)})});async open(e){if(_("[SQLiteWorkerHandler] open() called with config:",e),this.adapter)throw new Error("Database already open");this.bootDebug=e.bootDebug??!1,this.adapter=await de(e),_("[SQLiteWorkerHandler] open() completed"),this.bootDebug&&await this.logDbStats()}async logDbStats(){if(this.adapter)try{const[e,t,a,n]=await Promise.all([this.adapter.getDatabaseSize(),this.adapter.getStorageMode(),this.adapter.queryOne("PRAGMA page_count"),this.adapter.queryOne("PRAGMA freelist_count")]);x("[xNet] db stats @ open",{bytes:e,mode:t,pageCount:a?.page_count,freelistCount:n?.freelist_count})}catch{}}async resetStorage(e){this.adapter&&(await this.adapter.close(),this.adapter=null),await ie(e)}async close(){this.adapter&&(await this.adapter.close(),this.adapter=null)}isOpen(){return this.adapter?.isOpen()??!1}async query(e,t){if(!this.adapter)throw new Error("Database not open");return this.scheduler.schedule("interactive",()=>this.adapter.query(e,t),X("query",e,t),"query")}async queryOne(e,t){if(!this.adapter)throw new Error("Database not open");return this.scheduler.schedule("interactive",()=>this.adapter.queryOne(e,t),X("queryOne",e,t),"queryOne")}async run(e,t){if(!this.adapter)throw new Error("Database not open");return this.scheduler.schedule("write",()=>this.adapter.run(e,t),void 0,"run")}async exec(e){if(!this.adapter)throw new Error("Database not open");return this.scheduler.schedule("write",()=>this.adapter.exec(e),void 0,"exec")}async transaction(e){if(!this.adapter)throw new Error("Database not open");return this.scheduler.schedule("write",()=>this.adapter.transaction(async()=>{for(const t of e)await this.adapter.run(t.sql,t.params)}),void 0,"transaction")}async applyNodeBatch(e){if(!this.adapter)throw new Error("Database not open");return this.scheduler.schedule("write",()=>this.adapter.applyNodeBatch(e),void 0,"applyNodeBatch")}async getSchemaVersion(){if(!this.adapter)throw new Error("Database not open");return this.adapter.getSchemaVersion()}async vacuum(){if(!this.adapter)throw new Error("Database not open");return this.scheduler.schedule("write",()=>this.adapter.vacuum(),void 0,"vacuum")}async getSchedulerSnapshot(){return this.scheduler.snapshot()}async getDatabaseSize(){if(!this.adapter)throw new Error("Database not open");return this.adapter.getDatabaseSize()}async getStorageMode(){if(!this.adapter)throw new Error("Database not open");return this.adapter.getStorageMode()}connectPort(e){L(this,e)}},Ae=new Re;_("[SQLiteWorker] Handler instance created");_("[SQLiteWorker] Exposing handler via Comlink...");L(Ae);_("[SQLiteWorker] Handler exposed - worker ready!");
