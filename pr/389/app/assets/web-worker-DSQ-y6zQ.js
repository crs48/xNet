...
`,le=ce+de;function W(e){const t=e;return t?t.name==="NoModificationAllowedError"?!0:/Access Handles cannot be created|createSyncAccessHandle/i.test(t.message??""):!1}async function ue(e,t={}){const a=t.attempts??5,r=t.baseDelayMs??150,s=t.sleep??(o=>new Promise(c=>setTimeout(c,o)));let n;for(let o=1;o<=a;o++)try{return await e()}catch(c){if(n=c,!W(c)||o===a)throw c;t.onRetry?.(o,c),await s(r*o)}throw n}var he=64;function Ee(e){return e.replace(/[\s;]+$/,"").includes(";")}var pe=class{constructor(e=he){if(this.capacity=e,e<1)throw new Error("StmtCache capacity must be >= 1")}entries=new Map;get size(){return this.entries.size}get(e){const t=this.entries.get(e);return t!==void 0&&(this.entries.delete(e),this.entries.set(e,t)),t}set(e,t){const a=this.entries.get(e);for(a!==void 0&&a!==t&&this.safeFinalize(a),this.entries.delete(e),this.entries.set(e,t);this.entries.size>this.capacity;){const r=this.entries.keys().next().value,s=this.entries.get(r);this.entries.delete(r),s!==void 0&&this.safeFinalize(s)}}clear(){for(const e of this.entries.values())this.safeFinalize(e);this.entries.clear()}safeFinalize(e){try{e.finalize()}catch{}}},z="opfs-sahpool",D=".xnet-sqlite",Y=10;function Te(){return typeof self<"u"&&typeof localStorage<"u"&&localStorage.getItem("xnet:sqlite:debug")==="true"}function h(...e){Te()&&console.log(...e)}function p(){return typeof performance<"u"&&typeof performance.now=="function"?performance.now():Date.now()}function x(e){return e.path.startsWith("/")?e.path:`/${e.path}`}async function _e(e){const t=globalThis.navigator?.storage;if(typeof t?.getDirectory=="function")try{await(await t.getDirectory()).removeEntry(e.replace(/^\/+/,""),{recursive:!0})}catch(a){if(a instanceof DOMException&&a.name==="NotFoundError")return;throw a}}async function ye(e){const t=(await import("./index-DU5NF16A.js")).default,a=await t(),r=x(e);try{const s=await a.installOpfsSAHPoolVfs({name:z,directory:D,initialCapacity:Y,clearOnInit:!1});try{s.unlink(r)}catch{}await s.wipeFiles(),await s.removeVfs()}catch(s){console.warn("[WebSQLiteAdapter] SAH-pool reset failed, removing OPFS directory:",s),await _e(D)}}var Ne=class{sqlite3=null;db=null;poolUtil=null;_config=null;inTransaction=!1;lastTransactionActivityAt=0;storageMode="memory";openPhaseTimings=null;schemaApplyMs=0;openRetryAttempts=0;stmts=new pe;getOpenPhaseTimings(){return this.openPhaseTimings?{...this.openPhaseTimings}:null}async open(e){if(this.db!==null)throw new Error("Database already open. Call close() first.");h("[WebSQLiteAdapter] Starting open()...");const t=p();let a=t,r=t,s=t,n=t,o=t;h("[WebSQLiteAdapter] Importing sqlite-wasm...");const c=(await import("./index-DU5NF16A.js")).default;h("[WebSQLiteAdapter] sqlite-wasm imported"),a=p(),h("[WebSQLiteAdapter] Initializing sqlite3 module..."),this.sqlite3=await c(),h("[WebSQLiteAdapter] sqlite3 module initialized"),r=p();try{await ue(async()=>{h("[WebSQLiteAdapter] Installing OPFS-SAHPool VFS..."),this.poolUtil=await this.sqlite3.installOpfsSAHPoolVfs({name:z,directory:D,initialCapacity:Y,clearOnInit:!1}),h("[WebSQLiteAdapter] OPFS-SAHPool VFS installed"),s=p(),h("[WebSQLiteAdapter] Reserving capacity..."),await this.poolUtil.reserveMinimumCapacity(10),h("[WebSQLiteAdapter] Capacity reserved"),n=p();const i=x(e);h("[WebSQLiteAdapter] Opening database at",i),this.db=new this.poolUtil.OpfsSAHPoolDb(i,"c"),this.storageMode="opfs",h("[WebSQLiteAdapter] Database opened with OPFS-SAHPool"),o=p()},{onRetry:(i,l)=>{this.openRetryAttempts=i,console.warn(`[WebSQLiteAdapter] OPFS access handles are busy (attempt ${i}) — a previous tab/worker is likely still releasing them; retrying before any in-memory fallback.`,l)}})}catch(i){const l=se();l.mode==="async-opfs"?console.info("[WebSQLiteAdapter] Sync access handles unavailable — "+l.reason):console.warn("[WebSQLiteAdapter] OPFS-SAHPool not available, trying OPFS direct mode:",i);const T=x(e),U=this.sqlite3?.oo1?.OpfsDb;if(typeof U=="function")try{this.db=new U(T,"c"),this.storageMode="opfs",h("[WebSQLiteAdapter] Database opened with OPFS direct mode")}catch(R){h("[WebSQLiteAdapter] OPFS direct mode not available:",R),this.db=new this.sqlite3.oo1.DB(":memory:","c"),this.storageMode="memory"}else this.db=new this.sqlite3.oo1.DB(":memory:","c"),this.storageMode="memory";if(this.storageMode==="memory"){const R=W(i)?"another xNet tab/worker is holding the local database":"OPFS is unavailable in this browser context";console.error(`[WebSQLiteAdapter] Using an IN-MEMORY database (${R}). Local data will NOT persist across reloads and the workspace will appear empty until it re-syncs from the hub. Close other xNet tabs and reload to restore persistent storage.`)}}this._config=e;const E=p();e.foreignKeys!==!1&&this.execSync("PRAGMA foreign_keys = ON"),e.busyTimeout?this.execSync(`PRAGMA busy_timeout = ${e.busyTimeout}`):this.execSync("PRAGMA busy_timeout = 5000");try{this.execSync("PRAGMA page_size = 8192")}catch(i){h("[WebSQLiteAdapter] page_size pragma not applied:",i)}try{this.execSync("PRAGMA auto_vacuum = INCREMENTAL")}catch(i){h("[WebSQLiteAdapter] auto_vacuum pragma not applied:",i)}this.execSync("PRAGMA synchronous = NORMAL"),this.execSync("PRAGMA cache_size = -262144");try{this.execSync("PRAGMA mmap_size = 268435456")}catch(i){h("[WebSQLiteAdapter] mmap_size pragma not applied:",i)}this.execSync("PRAGMA temp_store = MEMORY");try{this.execSync("PRAGMA journal_mode = TRUNCATE")}catch(i){h("[WebSQLiteAdapter] journal_mode pragma not applied:",i)}try{this.execSync("PRAGMA analysis_limit = 400"),this.execSync("PRAGMA optimize = 0x10002")}catch(i){h("[WebSQLiteAdapter] open-time optimize not applied:",i)}const u=p(),d=(i,l)=>Math.round(l-i);this.openPhaseTimings={wasmImportMs:d(t,a),wasmInitMs:d(a,r),vfsInstallMs:d(r,s),reserveCapacityMs:d(s,n),dbOpenMs:d(n,o),pragmasMs:d(E,u),totalOpenMs:d(t,u)}}async close(){if(this.stmts.clear(),this.db){try{this.execSync("PRAGMA optimize")}catch(e){h("[WebSQLiteAdapter] optimize on close skipped:",e)}this.db.close(),this.db=null}this.sqlite3=null,this.poolUtil=null,this._config=null}isOpen(){return this.db!==null}getStorageMode(){return this.storageMode}isInTransaction(){return this.inTransaction}transactionIdleMs(){return this.inTransaction?p()-this.lastTransactionActivityAt:0}touchTransaction(){this.inTransaction&&(this.lastTransactionActivityAt=p())}getCachedStmt(e){if(Ee(e))return null;let t=this.stmts.get(e);return t===void 0&&(t=this.db.prepare(e),this.stmts.set(e,t)),t}async query(e,t){this.ensureOpen(),this.touchTransaction();const a=[],r=this.getCachedStmt(e);if(r===null)return this.db.exec({sql:e,bind:t,rowMode:"object",callback:s=>{a.push(s)}}),a;try{for(t&&t.length>0&&r.bind(t);r.step();)a.push(r.get({}))}finally{r.reset(),r.clearBindings()}return a}async queryOne(e,t){return(await this.query(e,t))[0]??null}async queryBatch(e){const t=[];for(const a of e)t.push(await this.query(a.sql,a.params));return t}async run(e,t){this.ensureOpen(),this.touchTransaction();const a=this.getCachedStmt(e);if(a===null)this.db.exec({sql:e,bind:t});else try{for(t&&t.length>0&&a.bind(t);a.step(););}finally{a.reset(),a.clearBindings()}return{changes:this.sqlite3.capi.sqlite3_changes(this.db.pointer),lastInsertRowid:this.sqlite3.capi.sqlite3_last_insert_rowid(this.db.pointer)}}async exec(e){this.ensureOpen(),this.stmts.clear(),this.execSync(e)}execSync(e){this.db.exec({sql:e})}async transaction(e){await this.beginTransaction();try{const t=await e();return await this.commit(),t}catch(t){if(this.inTransaction)try{await this.rollback()}catch(a){if(C(a)&&!C(t))throw a}throw t}}async applyNodeBatch(e){return await this.transaction(async()=>{for(const t of e.nodes)await this.run(`INSERT INTO nodes (id, schema_id, created_at, updated_at, created_by, deleted_at)
           VALUES (?, ?, ?, ?, ?, ?)
