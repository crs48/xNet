...
import { AGENT_LAYOUT_EVENT } from './agent-layout-event'
...
// never see it.
// Contributed lazily on first mount: at module scope the host adapter is not
// set yet (the barrel evaluates before the app's boot registration runs).
let tipsContributed = false
function contributeSwitcherTips(): void {
  if (tipsContributed) return
  tipsContributed = true
  workbenchHost().contributeTips([
    {
