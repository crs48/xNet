...
import { workbenchHost, type ProfileEntry } from '../host'
...
function AuthorChip({ did, profiles }: { did: string; profiles: ProfileEntry[] }) {
  const { colorForDid, displayName } = workbenchHost().comms
  return (
...
export function TimeMachinePanel({ nodeId }: { nodeId: string }) {
  ...
  workbenchHost().comms.useEnsureProfiles(authors)
  const profiles = workbenchHost().comms.useProfiles()
