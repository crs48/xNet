...
export function useWidgetData(request: WidgetDataRequest | undefined): {
  ...
  const { schemas, variables } = useDashboardRuntime()
  const intervalMs =
  ...
  const result = useSavedView(descriptor, schemas)

  const data = useMemo<WidgetData>(() => {
    if (!descriptor) return EMPTY_DATA
  ...
  }, [descriptor, result.primary, result.queries, result.loading, result.error])
