...
describe('signSlackRequest', () => {
  it('produces a stable v0= signature for the same inputs', () => {
    const a = signSlackRequest({ signingSecret: SECRET, timestamp: TS, rawBody: BODY })
    const b = signSlackRequest({ signingSecret: SECRET, timestamp: TS, rawBody: BODY })
    expect(a).toBe(b)
  ...
  it('changes when the body changes', () => {
    const a = signSlackRequest({ signingSecret: SECRET, timestamp: TS, rawBody: BODY })
    const b = signSlackRequest({ signingSecret: SECRET, timestamp: TS, rawBody: `${BODY}x` })
    expect(a).not.toBe(b)
  })
})
