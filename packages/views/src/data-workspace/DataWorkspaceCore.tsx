/**
 * DataWorkspaceCore — the shared Data Workspace surface (exploration 0276,
 * Theme 3: well-traveled code paths).
 *
 * The web and desktop DataWorkspaceViews were ~92%-identical drifted copies:
 * the same saved-view/descriptor parsing, workspace metrics, graph atlas,
 * pattern detection, seeding and lens-saving logic. This module owns:
 *
 * - `useDataWorkspace` — all queries, derived data, and handlers. Platform
 *   deltas arrive as options: `getExistingNode` (OPFS store vs. IPC bridge),
 *   `seedReady` (web gates on the store), and the desktop-only
 *   `onInsertSavedLensAsCanvasFrame` canvas integration.
 * - `DataWorkspaceBody` — the shared render (banners, import jobs, metrics,
 *   graph atlas, sources/patterns rail, saved-view tables, SavedViewRunner).
 *   Canvas-frame affordances (Frame / Pin buttons, visual-canvas projection)
 *   render only when the canvas callback is provided. Web-only SavedViewRunner
 *   extras (feed enrichment, moderation `wrapItem`) pass through
 *   `savedViewRunnerProps`.
 *
 * The app components keep only their own chrome: header, seed button
 * placement, close affordances, and scroll containers.
 */
...
/** Saved lens payload for the desktop canvas-frame integration. */
...
// ─── Dismissed patterns (localStorage) ─────────────────────────────────────────
...
export type UseDataWorkspaceOptions = {
  /** Resolve an existing node by deterministic id during seeding (platform storage differs). */
  getExistingNode: (id: string) => Promise<unknown>
  /** Gate seeding until the platform store is ready (web OPFS store). Default true. */
  seedReady?: boolean
  /** Desktop-only: insert a saved lens onto the canvas as a frame. */
  onInsertSavedLensAsCanvasFrame?: (input: SavedViewCanvasFrameInput) => void
}

export type UseDataWorkspaceResult = {
  // Seeding / status
  seedReady: boolean
  seeding: boolean
  seedSummary: SocialWorkspaceSeedSummary | null
  seedError: string | null
  saveLensMessage: string | null
  saveLensError: string | null
  handleSeedWorkspace: () => Promise<void>

  // Saved views
  savedViewsLoading: boolean
  savedViewCount: number
  defaultSeedCount: number
  importRunCount: number | null
  socialWorkspaceViews: SavedViewRow[]
  otherSavedViews: SavedViewRow[]
  selectedView: SavedViewRow | null
  setSelectedViewId: (viewId: string | null) => void

  // Derived data
  metrics: WorkspaceMetric[]
  analyticsCacheRecommendation: SocialAnalyticsCacheRecommendation
  patternSuggestions: SocialPatternSuggestion[]
  graphAtlasRows: GraphAtlasRow[]
  visibleSocialImportJobs: SocialImportJobProgress[]
  /** Re-read the module-level import-job store (e.g. after an IPC upsert). */
  refreshSocialImportJobs: () => void

  // Lens / pattern actions
  handleSaveLens: (draft: SavedViewLensDraft) => Promise<void>
  handleOpenPattern: (pattern: SocialPatternSuggestion) => void
  handleSavePattern: (pattern: SocialPatternSuggestion) => Promise<void>
  handlePinPattern: (pattern: SocialPatternSuggestion) => Promise<void>
  handleOpenVisualCanvasProjection: (request: SavedViewVisualCanvasProjectionRequest) => void
  handleDismissPattern: (patternId: string) => void

  /** Passed through so the body can render canvas-frame affordances. */
  onInsertSavedLensAsCanvasFrame?: (input: SavedViewCanvasFrameInput) => void
}
...
export function useDataWorkspace({
...
}: UseDataWorkspaceOptions): UseDataWorkspaceResult {
  ...
  return {
    ...
    defaultSeedCount: defaultSeeds.length,
    importRunCount: getCount(importRunQuery),
    socialWorkspaceViews,
    otherSavedViews,
    selectedView,
    setSelectedViewId,
    metrics,
    analyticsCacheRecommendation,
    patternSuggestions,
    graphAtlasRows,
    visibleSocialImportJobs,
    refreshSocialImportJobs,
    handleSaveLens,
    handleOpenPattern,
    handleSavePattern,
    handlePinPattern,
    handleOpenVisualCanvasProjection,
...
export type DataWorkspaceBodyProps = {
  workspace: UseDataWorkspaceResult
  /** Web-only SavedViewRunner extras: feed enrichment + moderation gate. */
  savedViewRunnerProps?: Pick<SavedViewRunnerProps, 'feedEnrichment' | 'wrapItem'>
}

/**
 * The shared Data Workspace surface: status banners, import jobs, metric
 * cards, graph atlas, and the sources/patterns + saved-views grid. The host
 * component provides page chrome (header, seed button, scroll container).
 */
export function DataWorkspaceBody({
  workspace,
  savedViewRunnerProps
}: DataWorkspaceBodyProps): ReactElement {
  const {
    seedSummary,
    seedError,
    saveLensMessage,
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                <SourceRow
                  ...
                  value={metricValueLabel(importRunCount)}
                />
                <SourceRow label="Saved views" value={String(savedViewCount)} />
                <SourceRow
                  label="Starter views"
                  value={`${socialWorkspaceViews.length}/${defaultSeedCount}`}
                />
...
function SocialImportJobsPanel({ jobs }: { jobs: SocialImportJobProgress[] }): ReactElement | null {
...
function JobMetric({ label, value }: { label: string; value: string }): ReactElement {
  return (
    <div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {rows.map((row) => (
          <GraphAtlasCard
            ...
            onOpen={onOpen}
            onInsertCanvasFrame={onInsertCanvasFrame}
          />
        ))}
      </div>
    </section>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {onInsertCanvasFrame ? (
          <button
            ...
            disabled={!savedView}
            onClick={() => {
              if (savedView) onInsertCanvasFrame(savedView)
            }}
            className="inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 text-xs transition-colors hover:bg-accent disabled:cursor-not-allowed disabled:opacity-45"
          >
            <Layout size={12} />
            Frame
          </button>
...
function GraphAtlasMetric({ label, value }: { label: string; value: number }): ReactElement {
  return (
...
function SourceRow({ label, value }: { label: string; value: string }): ReactElement {
  return (
