...
`,Q=B+Y,D="opfs-sahpool",A=".xnet-sqlite",X=10;function H(){return typeof self<"u"&&typeof localStorage<"u"&&localStorage.getItem("xnet:sqlite:debug")==="true"}function o(...e){H()&&console.log(...e)}function w(e){return e.path.startsWith("/")?e.path:`/${e.path}`}async function z(e){const t=globalThis.navigator?.storage;if(typeof t?.getDirectory=="function")try{await(await t.getDirectory()).removeEntry(e.replace(/^\/+/,""),{recursive:!0})}catch(a){if(a instanceof DOMException&&a.name==="NotFoundError")return;throw a}}async function K(e){const t=(await import("./index-BdoYNwm6.js")).default,a=await t(),s=w(e);try{const r=await a.installOpfsSAHPoolVfs({name:D,directory:A,initialCapacity:X,clearOnInit:!1});try{r.unlink(s)}catch{}await r.wipeFiles(),await r.removeVfs()}catch(r){console.warn("[WebSQLiteAdapter] SAH-pool reset failed, removing OPFS directory:",r),await z(A)}}var V=class{sqlite3=null;db=null;poolUtil=null;_config=null;inTransaction=!1;storageMode="memory";async open(e){if(this.db!==null)throw new Error("Database already open. Call close() first.");o("[WebSQLiteAdapter] Starting open()..."),o("[WebSQLiteAdapter] Importing sqlite-wasm...");const t=(await import("./index-BdoYNwm6.js")).default;o("[WebSQLiteAdapter] sqlite-wasm imported"),o("[WebSQLiteAdapter] Initializing sqlite3 module..."),this.sqlite3=await t(),o("[WebSQLiteAdapter] sqlite3 module initialized");try{o("[WebSQLiteAdapter] Installing OPFS-SAHPool VFS..."),this.poolUtil=await this.sqlite3.installOpfsSAHPoolVfs({name:D,directory:A,initialCapacity:X,clearOnInit:!1}),o("[WebSQLiteAdapter] OPFS-SAHPool VFS installed"),o("[WebSQLiteAdapter] Reserving capacity..."),await this.poolUtil.reserveMinimumCapacity(10),o("[WebSQLiteAdapter] Capacity reserved");const a=w(e);o("[WebSQLiteAdapter] Opening database at",a),this.db=new this.poolUtil.OpfsSAHPoolDb(a,"c"),this.storageMode="opfs",o("[WebSQLiteAdapter] Database opened with OPFS-SAHPool")}catch(a){console.warn("[WebSQLiteAdapter] OPFS-SAHPool not available, trying OPFS direct mode:",a);const s=w(e),r=this.sqlite3?.oo1?.OpfsDb;if(typeof r=="function")try{this.db=new r(s,"c"),this.storageMode="opfs",o("[WebSQLiteAdapter] Database opened with OPFS direct mode")}catch(n){console.warn("[WebSQLiteAdapter] OPFS direct mode not available, using in-memory database:",n),this.db=new this.sqlite3.oo1.DB(":memory:","c"),this.storageMode="memory",o("[WebSQLiteAdapter] In-memory database opened")}else console.warn("[WebSQLiteAdapter] OPFS direct mode unavailable, using in-memory database"),this.db=new this.sqlite3.oo1.DB(":memory:","c"),this.storageMode="memory",o("[WebSQLiteAdapter] In-memory database opened")}this._config=e,e.foreignKeys!==!1&&this.execSync("PRAGMA foreign_keys = ON"),e.busyTimeout?this.execSync(`PRAGMA busy_timeout = ${e.busyTimeout}`):this.execSync("PRAGMA busy_timeout = 5000");try{this.execSync("PRAGMA page_size = 8192")}catch(a){o("[WebSQLiteAdapter] page_size pragma not applied:",a)}this.execSync("PRAGMA synchronous = NORMAL"),this.execSync("PRAGMA cache_size = -262144"),this.execSync("PRAGMA temp_store = MEMORY");try{this.execSync("PRAGMA journal_mode = TRUNCATE")}catch(a){o("[WebSQLiteAdapter] journal_mode pragma not applied:",a)}}async close(){if(this.db){try{this.execSync("PRAGMA optimize")}catch(e){o("[WebSQLiteAdapter] optimize on close skipped:",e)}this.db.close(),this.db=null}this.sqlite3=null,this.poolUtil=null,this._config=null}isOpen(){return this.db!==null}getStorageMode(){return this.storageMode}async query(e,t){this.ensureOpen();const a=[];return this.db.exec({sql:e,bind:t,rowMode:"object",callback:s=>{a.push(s)}}),a}async queryOne(e,t){return(await this.query(e,t))[0]??null}async run(e,t){return this.ensureOpen(),this.db.exec({sql:e,bind:t}),{changes:this.sqlite3.capi.sqlite3_changes(this.db.pointer),lastInsertRowid:this.sqlite3.capi.sqlite3_last_insert_rowid(this.db.pointer)}}async exec(e){this.ensureOpen(),this.execSync(e)}execSync(e){this.db.exec({sql:e})}async transaction(e){await this.beginTransaction();try{const t=await e();return await this.commit(),t}catch(t){if(this.inTransaction)try{await this.rollback()}catch(a){if(R(a)&&!R(t))throw a}throw t}}async applyNodeBatch(e){return await this.transaction(async()=>{for(const t of e.nodes)await this.run(`INSERT INTO nodes (id, schema_id, created_at, updated_at, created_by, deleted_at)
           VALUES (?, ?, ?, ?, ?, ?)
