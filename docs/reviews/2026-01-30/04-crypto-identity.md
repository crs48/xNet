...
Review of `@xnetjs/crypto` and `@xnetjs/identity` packages covering cryptographic primitives, key management, DID:key identity, and UCAN token handling.
...
| m-04 | Unused dependency `@xnetjs/core` in `package.json`                                            | `package.json:23` |
...
| I-09 | Unused dependency `@xnetjs/core`                                        | `package.json:20` |
...
| I-11 | Duplicated base64url utilities (4 functions) already exist in `@xnetjs/crypto`  | `ucan.ts:134-162`   |
| I-12 | Duplicated `concatBytes` already in `@xnetjs/crypto`                            | `passkey.ts:95-104` |
...
**Recommendation:** All packages should import these utilities from `@xnetjs/crypto` rather than re-implementing them. This eliminates bug duplication (the spread operator issue exists in 3+ copies).
...
- [ ] **I-11:** Deduplicate base64url/concatBytes from `identity/ucan.ts` and `identity/passkey.ts` -- import from `@xnetjs/crypto`
...
- [ ] **m-04/I-09:** Remove unused `@xnetjs/core` dependency from both `crypto` and `identity`
