...
### Load-bearing storage constraint (found during implementation, 0298)

`node_changes` uses **`hash` as a global PRIMARY KEY** with `INSERT OR IGNORE`,
and `hasNodeChange(hash)` dedups **globally** (`packages/hub/src/storage/sqlite.ts:321,1161`).
So **a change physically lives in exactly one room** — you cannot "copy" a
channel's existing changes into an `xnet-channel-<id>` room (the second insert
is silently ignored; the content hash is identical). Two viable shapes:

1. **Change↔room mapping** — add `node_change_rooms(hash, room, seq)` so a
   canonical change can be indexed into additional rooms without duplication.
   `getNodeChangesSince(room, …)` reads via the mapping. **Cursor caveat:** the
   client's `NodeStoreSyncProvider` advances a `sinceLamport` high-water mark per
   room, but a channel room aggregates changes from many authors whose lamports
   are not mutually ordered — so the mapping must carry a **per-room monotonic
   `seq`** (hub insertion order into that room) and the channel-room sync must
   cursor on `seq`, not the author lamport, or late-arriving low-lamport changes
   get skipped. Delivery stays idempotent because the client applies by hash.
2. **Channel-as-home room** — author the `Channel`/`ChatMessage` nodes into
   `xnet-channel-<id>` from the start (Matrix model). Cleanest long-term but
   changes the comms write path and requires the owner to subscribe to all their
   channel rooms to keep `useChannels()` populated.

This constraint is why "route into a room" is not a one-liner and why the
implementation is storage-touching. The mapping approach (1) is the
least-invasive correct foundation and is what this exploration recommends
building first.
...
- **DMs are shareable** — do _not_ exclude `kind: 'dm'`. Sharing a DM shares
  that conversation like any channel.
- **Member identity travels with the share** — "if a channel is shared with
  you, you also get the names and profiles of the people in it." The hub fans
  out each member's `Profile` node (and each message author's `Profile`) into
  the channel room so names/avatars render for the grantee.
- **No backfill** — pre-release; do not migrate historical channels/messages.
  New shares populate their room going forward; the owner's client (which holds
  the channel + its messages + member profiles locally) is the population source
  on first share, and the hub fans out subsequent activity.
- **Owner channel list** — dual-index the `Channel` node into both the owner's
  author room and the channel room (least-risk; the owner keeps `useChannels()`
  without subscribing to N rooms).
- **Workspaces** — Phase 2 (bench node only; referenced docs need their own
  shares). Not in the first PR.
