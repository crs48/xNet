...
- [x] Gate everything behind `localStorage['xnet:trace'] === '1'` (dev-only default) — wired in apps/web (`lib/tracing.ts`); same collector handed to `config.tracing` and `XNetDevToolsProvider.traceCollector`.
