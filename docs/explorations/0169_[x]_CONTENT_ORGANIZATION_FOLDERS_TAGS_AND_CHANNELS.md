...
- [x] Include tag names in hub FTS `index-update` metadata (the producer resolves tag ids to `#name` search text and uses `name` as the title for title-less schemas)
...
- [x] Schema unit tests: folder nesting, cycle prevention on `parent` (a folder cannot be its own ancestor), tag name normalization
- [x] `extractTagIds` tests mirroring `mentions.ts` tests: pills extracted, raw `#text` in prose ignored
- [x] Sort: sibling reorder uses `compareSortKeys` (code-unit), verified by code-unit-vs-locale tests in `folder.test.ts` and `explorer-folders.test.ts`
- [ ] e2e (Playwright) folder flow — create folder → rename → move item in → nested render → breadcrumb was verified manually in a live browser this round; an automated spec is deferred
- [ ] e2e tag flow — type `#` → create tag → pill inserted → tag page lists the page and hosts a discussion channel, verified manually in a live browser; automated spec + rename-propagation assertion deferred
- [ ] Concurrent-move test: convergence holds by construction (single-property LWW on `folder`), but a two-client harness test is deferred
- [ ] Perf: tag-page query over 10k nodes stays within the 0163 hot-path budgets — not yet measured
- [ ] Unfiled ratio observable: count of homeless nodes exposed in dev tooling — deferred
- [ ] UX check: sharing messaging — folder delete states "contents move up", but explicit "folders don't change visibility" copy is deferred
