...
> [!TIP]
> **TL;DR** — Do **both, asymmetrically**. Go deep on being the *substrate*
> agents run against (signed identity, scoped delegation, tamper-evident
> audit — the accountability layer none of them have), finish the *client*
...
- **The layer *below* it is empty.** OpenClaw's audit is mutable JSONL under
...
- **`Buzz` is the one real thesis competitor**, and it is adjacent rather than
  ...
  git forge over Smart HTTP and full self-hosting. It takes the *identity*
...
this by adding a log: the approval's *provenance* is enforced by which key
...
> that did **not** commoditise is the one that answers *"what did the agent
...
- **Buzz** is the exception and deserves credit: agents carry cryptographic
  keypairs, so authorship is real. But the record is a *chat/forge event log*,
...
> as *the Buzz idea* through Block's distribution, xNet's 0337 work — which
...
7. **The dangerous adjacency is retrieval, not orchestration.** Per 0379,
   ...
   can exfiltrate everything. This argues for investment in the *governed
   read path*, which is lane 1 work, not lane 3 work.
...
> agent *actions* is likewise out: it charges the user for using their own
...
positioning is free: *your Buzz agent can work in your xNet workspace, and
every change it makes lands in a ledger Buzz does not have.*
...
*any* externally-identified agent into an enrolled xNet identity, so the
...
| --- | --- | --- |
| "Accountability substrate" is a category users don't ask for | 🔴 High | Lead with the agent experience (lane 2), let audit be the reason it's *safe*; never lead with the ledger |
...
- Should notarisation be a hub role (0382 "everything is a hub, named roles")
  or a distinct service? 0382's rule — split by *authority*, not weight —
...
- [ ] Write ADR "xNet is not an agent harness" in `docs/decisions.mdx`,
      citing the June 2026 meta-harness wave, and linking this exploration
- [ ] Add the positioning line ("Bring your own agent…") to
