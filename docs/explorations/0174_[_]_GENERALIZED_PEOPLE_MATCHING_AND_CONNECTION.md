...
performance of self. Each forces you to author a *new* lossy profile — a bio, a
...
friendship. But to *get* there on today's platforms you must first broadcast.
...
*facets of the same mechanism* — and does it in a **decentralized, federated,
local-first** way that turns xNet's biggest asset (it already holds a rich,
user-owned graph of who you are) into serendipitous, intimate, *low-performance*
introductions?
...
listen to. The product is a consent-gated, reciprocal, double-opt-in *matchmaker*
...
Nostr, Farcaster, Scuttlebutt, Mastodon) keeps its *data* distributed but its
*discovery* concentrated: ATProto's AppView, Farcaster's Hub aggregator,
...
that *mutual* predicted interest (harmonic mean of both directions) is the right
...
differentiator is that it can build the match profile *from the graph it already
holds* and **explain** every introduction ("you overlap deeply on Rust +
...
1. **`ConnectableProfile` + `ConnectionIntent`** — an opt-in, intent-scoped
   ...
   This is the *only* new user-facing concept; it generalizes dating →
...
3. **Hub matchmaker (the AppView)** — a federated directory that indexes only
   *consented, coarsened* signals and runs **reciprocal scoring + graph
...
xNet already ships ~80% of the substrate. The feature is mostly *assembly*, not
greenfield.
...
| **Imported actor** | `SocialActorSchema` ([actor.ts:16](packages/social/src/schemas/actor.ts)) | `platform`, `actorKind`, `handle`, `displayName`, `privacyClass`, `visibility`, `metadataJson` | Describes *other* people you've imported; rich but not "you" |
...
graph.** The matching profile can be *derived and explained*, not authored.
...
- **Federated query router is a typed stub.** `createFederatedQueryRouter`
  ...
  is *the* seam to extend for cross-hub people search.
...
lesson, verbatim from the research: *"A decentralized system with centralized
defaults is a centralized system — just with extra latency."* **For matching,
...
- **Tinder's ELO/desirability score** ranked people globally; abandoned because
  it was gameable, stratifying, and *unilateral* (measures who wants you, not
  mutual fit). Don't build a global desirability rank.
- **Hinge "Most Compatible"** uses **Gale-Shapley stable matching** over
  *predicted mutual* attraction and weights effortful comments above likes —
...
- **Marketplace pathologies** (Bruch & Newman, *Science Advances* 2018):
...
**activity-based** matching (Polywork: match on what you *do*, not your title).
...
- **Content-based** (SBERT/MiniLM cosine over bios/interests) solves *content*
...
- **Graph link-prediction:** Common Neighbors, **Adamic-Adar** (down-weights
  ...
  on the *local* subgraph — no global index needed → perfect for friend-of-
  friend).
...
Granovetter's *Strength of Weak Ties* (1973) and its causal confirmation
(Rajkumar et al., *Science* 2022, on LinkedIn) show **bridging weak ties carry
...
UCB** for exploration, and the heuristic *"overlapping core interest + different
community"* — enough common ground to connect, enough difference to surprise.
...
**shared blocklists** (Fediseer, ATProto Ozone labelers — *which xNet's `abuse`
package already mirrors*), and **double opt-in as the non-negotiable first-
...
1. **xNet is unusually well-suited to this** precisely because the hard part of
   ...
   *projection*, not a questionnaire.
...
- **Cold-start at the network level.** A new hub has nobody. Mitigation: Phase 1
  works purely on your *existing* owned graph (friends-of-friends) — value
...
- **Gamification creep.** The moment this gets swipe streaks and notifications it
  ...
  *connection quality* (Lunchclub-style post-intro feedback) instead of
