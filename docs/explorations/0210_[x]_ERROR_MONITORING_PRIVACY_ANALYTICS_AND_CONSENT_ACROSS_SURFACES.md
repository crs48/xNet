...
> Status: IMPLEMENTED (P0–P4) — see the checklist below. Deferred: installing the
> actual `@sentry/*` SDKs + minting DSNs (the seams are wired and load them
> dynamically/by CDN), and the full first-party hub-ingest transport for app
> crashes (0187).
> Date: 2026-06-21
...
- [x] **P0:** Add `window.onerror` + `unhandledrejection` handlers in
  [apps/web/src/main.tsx](apps/web/src/main.tsx) before storage/SQLite boot
  ([boot-diagnostics.ts](apps/web/src/lib/boot-diagnostics.ts)).
- [x] **P0:** Add a boot timeout → actionable "couldn't start" screen (read
  `__xnetBootError`) instead of an indefinite spinner (`boot-timeout` state +
  blank-screen DOM fallback).
- [x] **P0:** Expose the furthest boot phase as `lastBootPhase()` /
  [boot-timeline.ts](apps/web/src/lib/boot-timeline.ts) and stamp it on failures.
- [ ] **P1:** Pick errors vendor (Sentry EU vs. self-host GlitchTip) and create
  the project; mint DSN. *(deferred — operator step; seams wired.)*
- [x] **P1:** Browser Sentry behind `VITE_XNET_TELEMETRY` + consent ≥ `crashes`,
  `sendDefaultPii:false`, `beforeSend` scrub — via the CDN loader
  ([sentry.ts](apps/web/src/lib/sentry.ts)), no npm dep until opted in. *(`tunnel`
  subdomain deferred.)*
- [x] **P1:** `app.onError()` + structured logging in `apps/cloud` + a dynamic
  `@sentry/node` bridge; `SENTRY_DSN` added to
  [cloud-secrets-push.mjs](scripts/cloud-secrets-push.mjs) +
  [deploy-cloud.yml](.github/workflows/deploy-cloud.yml). *(Used a zero-dep JSON
  logger instead of `pino`.)*
- [ ] **P1:** First-party tunnel subdomain (`i.xnet.fyi`) via Cloudflare Worker.
  *(deferred.)*
- [x] **P1:** Sentry/analytics disabled on PR/branch previews and self-host
  (env-gated; previews build only `apps/web` without the flags).
- [x] **P2:** Analytics = Plausible by default, self-host-swappable via
  `PUBLIC_ANALYTICS_SRC` / `VITE_ANALYTICS_SRC`. *(First-party reverse proxy
  deferred.)*
- [x] **P2:** Inject the cookieless script in
  [Base.astro](site/src/layouts/Base.astro), the docs
  [Head.astro](site/src/components/docs/Head.astro), and the app shell, env-gated.
- [ ] **P2:** Route product events (boot-ok, onboarding-done, first-sync) through
  `@xnetjs/telemetry` at `anonymous` tier. *(deferred — collector + Sentry path
  shipped; richer product events later.)*
- [ ] **P3:** Close the first-party pipeline gaps from 0187 (persist collector,
  real HTTP transport to the hub) so the app + hub share one ingest. *(deferred —
  collector captures locally; hub transport is 0187's job.)*
- [x] **P3:** Build the "Privacy & Diagnostics" panel in
  [settings.tsx](apps/web/src/routes/settings.tsx) over the consent spine.
- [x] **P3:** Lightweight first-run consent banner
  ([ConsentBanner.tsx](apps/web/src/components/ConsentBanner.tsx)); reporter +
  analytics subscribe to the same `ConsentManager`.
- [x] **P3:** Update [privacy.astro](site/src/pages/privacy.astro) to name
  Plausible + the opt-in app crash reporting. *(terms.astro unchanged — no new
  obligations.)*
- [x] **P4:** `diagnosticsSharingFeature` hub feature (scoped env) forwarding
  scrubbed, content-free crashes; default off
  ([diagnostics-sharing.ts](packages/hub/src/features/diagnostics-sharing.ts)).
- [x] **P4:** Document the opt-in in the hub setup guide
  ([hub.mdx](site/src/content/docs/docs/guides/hub.mdx)). *(Managed-hub DPA
  deferred to the 0128 compliance track.)*
