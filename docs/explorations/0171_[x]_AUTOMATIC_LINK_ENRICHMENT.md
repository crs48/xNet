...
- [x] Chat: send a message containing a URL, a bare domain, and an email →
      all clickable; clicking does not trigger any composer/row interaction;
      `(edited)` and redaction states unaffected
- [x] Table: type a URL into a plain text cell, press Enter → read mode shows
      a clickable link; clicking opens a new tab and does **not** enter edit
      mode; double-click still edits and shows raw text
- [x] Pages: typing `example.com ` autolinks with `https://`; undo removes
      the link mark and keeps text
- [x] Comments: existing GFM linkification unchanged (regression check)
- [x] Links open with `rel="noopener noreferrer"`; `mailto:`/`tel:` have no
      `target="_blank"`
- [x] Perf: scroll a long channel (500+ messages) — no measurable render
      regression vs. baseline
- [x] FTS/search: message and cell text remain searchable as raw text (no
