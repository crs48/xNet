...
began as a *Warcraft III* custom map and spawned the entire MOBA genre. That
...
- **The mod era did not die of natural causes; it was locked out by
  ...
  mod; Team Fortress (1996) a Quake mod; DotA (2003) a *Warcraft III* map
  descended from StarCraft's *Aeon of Strife*; the battle-royale genre came out
...
- **Every surviving moddable ecosystem has a safety story, and the ones that
  ...
  extensions in a separate host process. The counterexample is *fractureiser*
...
- **"The app is a view over your data" is the structural precondition of
  ...
  Shirky's *situated software* (2004), Sloan's *home-cooked meal* (2020), Ink &
  Switch's *malleable software* (2025), and Obsidian's *file over app* all
...
- **Recommendation:** Ship **blog post #10** at `site/src/pages/blog/`,
  ...
  capability-scoped views-over-data as the synthesis: moddability *and* safety,
...
- `packages/data/src/schema/extension.ts`, `lens.ts`, `sidecar.ts` — users
  ...
  `extension-resolver.ts`. Modding the *data shape*, not just the UI.
...
- `packages/devkit/src/bridge.ts` — the `:31416` agent bridge: your own
  Claude Code / Codex builds mods *for* your workspace, in worktrees, behind
  validation gates. This is the AI-age "home-cooked meal" kitchen.

### The publishing surface

- `site/src/pages/blog/` — nine hand-authored `.astro` posts (newest: *Hand
  on the Tiller*, 2026-07-03), metadata centralized in
...
website). This exploration is the *narrative* capstone over that series — the
...
| ~2002 | *Aeon of Strife*, a StarCraft custom map (authorship/date vary by source — hedge in the essay) | Genre seed |
| 2003 | Eul ports the concept to *Warcraft III: Reign of Chaos* as **Defense of the Ancients**; *DotA Allstars* flourishes on *The Frozen Throne* under Guinsoo, then IceFrog | The user's anecdote, verified: DotA was a Warcraft III mod |
...
- **Manifest V3**: Chrome replaced blocking `webRequest` with static
  ...
  gorhill's critique: the removed capability is what *protective* extensions
...
- **Ink & Switch, "Malleable software" (2025, Litt / Horowitz / van
  ...
  *Local-first software*, 2019; *End-user Programming*.)
...
- **Figma plugins** — started with the Realms shim; after a disclosed
  ...
  allowlisted APIs. (Figma blog: *How to build a plugin system on the web and
  also sleep well at night*, 2019.)
...
4. **The AI angle inverts the maintenance objection.** The classic case
   ...
   bridge (`packages/devkit/`) the marginal cost of both *making* and
   *maintaining* a personal view collapses — but only if the substrate is
...
5. **The essay is the tenth post and the capstone of a series-within-a-series.**
   ...
   this one explains *why the machine is shaped that way*. It should
...
the essay's best exhibit because a reader can *see* the guardrails.
...
1. **Cold open — the map that became a genre.** A custom map for *Warcraft
   III* — made by pseudonymous modders named Eul, Guinsoo, IceFrog — became
...
- **Overclaiming the marketplace.** The registry, licensing, and install gate
  ...
  must present xNet as *architecture for* the workshop, not as a bustling one.
- **Game-history pedantry.** MOBA-lineage readers are exacting. Hedge Aeon of
  ...
  Eul→Guinsoo→IceFrog chain, note DotA started on *Reign of Chaos* and
  flourished on *The Frozen Throne*. All dates above were verified against
  multiple sources.
...
- [ ] Draft `site/src/pages/blog/the-workshop-and-the-walled-garden.astro`
      following the six-beat structure above (en-GB, calm/evergreen voice).
- [ ] Add the `BlogPost` entry to `site/src/data/blog.ts` (post #10; RSS picks
      it up automatically via `site/src/pages/rss.xml.ts`).
- [ ] Reuse post #7's `Mermaid` + `CodeFigure` components: one lineage or
      install-flow diagram, the manifest CodeFigure, at most one more.
- [ ] Cross-link posts #7 (internals) and #8 (developer deep-dive), and the
      malleable-software / situated-software / home-cooked-meal sources.
- [ ] Fact-check pass against the verified table in this doc (dates, names,
      the Aeon of Strife hedge).
- [ ] Art direction: 2–3 comps in the established style (suggestion: a workshop
...
- [ ] `pnpm` site build + preview screenshot; humane-patterns lint
...
- Ink & Switch, *Local-first software* (2019) — <https://www.inkandswitch.com/essay/local-first/>
- Ink & Switch, *End-user Programming* — <https://www.inkandswitch.com/end-user-programming/>
- Clay Shirky, *Situated Software* (2004) — <http://shirky.com/essays/situated-software/>
- Robin Sloan, *An app can be a home-cooked meal* (2020) — <https://www.robinsloan.com/notes/home-cooked-app/>
- Steph Ango, *File over app* (2023) — <https://stephango.com/file-over-app>
- Malleable Systems Collective — <https://malleable.systems/>
- Solid project — <https://solidproject.org/>
- Geoffrey Litt, *Malleable software in the age of LLMs* (2023) — <https://www.geoffreylitt.com/2023/03/25/llm-end-user-programming.html>
