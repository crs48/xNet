...
- [ ] `resolve-prs.mjs`: add the GitHub-API fallback (`commits/{addSha}/pulls`)
      after the git-subject heuristics; derive `REPO` from `GITHUB_REPOSITORY`.
- [ ] `resolve-prs.mjs`: collect unresolved ids → `::warning` annotations + a
      summary log line (fail-open, never block).
- [ ] Call `resolve-prs.mjs` in `deploy-pr-preview.yml` and
      `deploy-branch-preview.yml` (both need `fetch-depth: 0`).
- [ ] `new.mjs`: `--pr auto` (default) bakes `pr` via `gh pr view` when a PR
      exists; best-effort, silent skip otherwise.
- [ ] `new.mjs`: stamp the date from the local day (or `--date`) to fix the
      UTC-tomorrow drift.
- [ ] `index.astro`: render a "View PR #N →" affordance + "no screenshots" note
      for entries with `pr` but no gallery items.
- [ ] Update the stale comment in `changelog.ts` ("CI writes one automatically") to
      describe the real author-writes-fragment + deploy-resolves-number flow.

## Validation Checklist

- [ ] A fragment merged via **rebase-merge** (no `#N` in any subject) still
      resolves to its PR at deploy (API path).
- [ ] Removing the merge-subject `#N` locally, `resolve-prs.mjs` still resolves via
      the API and exits 0.
- [ ] An intentionally unresolvable fragment produces a `::warning` + log line and
      the entry still renders (bare) — nothing silent.
- [ ] PR-preview build of `/changelog/` shows `#N` links for already-merged
      entries; the in-PR new entry is unresolved but visibly flagged.
- [ ] `new.mjs --pr auto` on a branch with an open PR writes `pr` into the fragment
      (one commit); on a branch with no PR it omits `pr` and prints a hint.
- [ ] An entry with `pr` but no visual manifest shows a "View PR #N" link (not a
      bare block); the feed still carries `_xnet.pr`.
- [ ] `new.mjs` authored at 22:00 local stamps **today's** date.
- [ ] Live re-check: every entry on `/changelog/` has a `pull/<N>` link and every
      `changelog.json` item has `_xnet.pr` (regression guard for this whole area).
