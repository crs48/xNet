...
- Add `idx_nodes_live_schema_updated` and `idx_nodes_all_schema_updated` to SQLite schema migrations.
- Use the existing `listNodesOptimized` join path for SQLite list hydration or promote it into the `NodeStorageAdapter` interface.
- Push `limit` and `offset` from `DataBridge` into `store.list` only when the descriptor has no property filters, no spatial filter, and default or system-field ordering compatible with SQLite.
- Add query timing and hydrated-row-count telemetry around bridge `loadQuery`.
- Add `EXPLAIN QUERY PLAN` assertions for default list queries in SQLite adapter tests.

Expected impact: simple lists and paginated lists stop hydrating full schemas.

### Phase 1: Scalar Property Index

- Add `node_property_scalars` schema and migration.
- Maintain scalar rows in `SQLiteNodeStorageAdapter._setNodeInternal` after node upsert.
- Delete scalar rows for node properties that are removed from `NodeState.properties`.
- Add a rebuild utility that scans `node_properties`, decodes values in JS, and repopulates the sidecar table.
- Add tests for text, number, boolean, null, deleted nodes, property deletion, and import paths.
- Keep all existing query results verified by `applyQueryDescriptor`.
...
- Add an optional storage capability such as `queryNodes(descriptor)` without breaking memory storage.
- Let `NodeStore` or `DataBridge` detect the capability and fall back to `list` when absent.
- Compile supported descriptors into candidate SQL plus batch hydration SQL.
- Return plan metadata: rows scanned, IDs returned, SQL used, indexes detected, post-filter reason.
...
- [ ] `useQuery` results match `applyQueryDescriptor` exactly for all supported descriptors.
- [ ] SQL plans never omit JS post-filtering unless parity tests prove exact behavior.
- [ ] Deleted node semantics match `includeDeleted` behavior.
- [ ] `limit` and `offset` produce identical results for default list, `createdAt`, and `updatedAt` ordering.
- [ ] Property equality handles string, number, boolean, null, and missing values correctly.
- [ ] Property sorting matches JS null/undefined behavior or remains post-sorted in JS.
- [ ] Scalar sidecar rebuild produces byte-for-byte equivalent query results before and after rebuild.
- [ ] Import, remote sync, restore, and transaction paths update sidecar rows.
- [ ] Encrypted node properties are not indexed in plaintext by accident.
- [ ] Auth-sensitive reads do not expose unauthorized counts or IDs.
- [ ] `EXPLAIN QUERY PLAN` shows expected indexes for default list and hot property filters.
...
- [ ] Electron IPC path avoids serializing full schema results for paginated reads.
- [ ] Query diagnostics are visible enough to debug slow user workspaces.
