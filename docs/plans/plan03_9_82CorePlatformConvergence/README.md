...
| Background sync lifecycle | [`packages/sync/src/sync-runtime.ts`](../../../packages/sync/src/sync-runtime.ts), [`packages/react/src/sync/sync-manager.ts`](../../../packages/react/src/sync/sync-manager.ts), and [`apps/electron/src/renderer/lib/ipc-sync-manager.ts`](../../../apps/electron/src/renderer/lib/ipc-sync-manager.ts) now share explicit `idle`/`starting`/`local-ready`/`connecting`/`healthy`/`degraded`/`replaying`/`stopped` phases with reconnect/replay/presence tests, but signed-by-default relay behavior is still not converged                                                                                                                                                                                             | sync observability and lifecycle semantics are better, yet the security cutover and real multi-device proof remain open                                                        |
