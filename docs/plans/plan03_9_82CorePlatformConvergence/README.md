...
| Database editing model  | [`packages/react/src/index.ts`](../../../packages/react/src/index.ts) exports a node-centric database hook family; [`apps/web/src/components/DatabaseView.tsx`](../../../apps/web/src/components/DatabaseView.tsx) and [`apps/electron/src/renderer/components/DatabaseView.tsx`](../../../apps/electron/src/renderer/components/DatabaseView.tsx) now both consume that hook layer; [`packages/data/src/database/legacy-migration.ts`](../../../packages/data/src/database/legacy-migration.ts) provides explicit one-way materialization with status recording; structured undo and cross-device proving are still outstanding | API clarity is substantially better now, but undo semantics and multi-device correctness still need explicit proof before the database layer is truly finished |
