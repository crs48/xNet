...
| Database editing model  | [`packages/react/src/index.ts`](../../../packages/react/src/index.ts) exports a node-centric database hook family; [`apps/web/src/components/DatabaseView.tsx`](../../../apps/web/src/components/DatabaseView.tsx) now consumes that hook layer via a temporary legacy adapter; [`packages/data/src/database/legacy-migration.ts`](../../../packages/data/src/database/legacy-migration.ts) now provides explicit one-way materialization with status recording; [`apps/electron/src/renderer/components/DatabaseView.tsx`](../../../apps/electron/src/renderer/components/DatabaseView.tsx) still writes whole `rows` and `columns` arrays into one Y.Map | API clarity, undo semantics, and multi-device correctness stay ambiguous until Electron and the migration boundary converge |
