...
class YjsSuperPeer {
  ...
  private handleYjsUpdate = (docId: string, update: Uint8Array) => {
    ...
    this.libp2p.services.pubsub.publish(`yjs/${docId}`, update)
  }
...
_RPO = Recovery Point Objective (max data loss)_
_RTO = Recovery Time Objective (time to restore)_
...
class SnapshotManager {
  ...
  async restoreSnapshot(id: string): Promise<void> {
    ...
    if (dataLoss.documents > 0) {
      throw new Error(
        `Restore would lose ${dataLoss.documents} documents. ` + `Use forceRestore() to proceed.`
      )
    }
...
class ProceduralEdgeComputer {
  ...
  computeSharedAttributeEdges(items: DatabaseItem[], propertyId: string): Edge[] {
...
class AllViewCanvas {
  ...
  generate(filters: EdgeFilterConfig, layout: LayoutConfig): CanvasDocument {
    ...
    if (layout.algorithm !== 'manual') {
      const positions = this.layoutEngine.layout(Array.from(nodes.values()), visibleEdges, layout)
  ...
  private computeVisibleEdges(filters: EdgeFilterConfig): Edge[] {
    ...
    if (filters.procedural.tags) {
      const allItems = [...this.pages.values(), ...this.items.values()]
...
function CanvasView({ document, onNodeMove, onNodeEdit }: CanvasViewProps) {
  ...
  const visibleNodes = useMemo(() => spatialIndex.queryWithLOD(viewport), [spatialIndex, viewport])

  // Get visible edges
  const visibleEdges = useMemo(() => {
    const nodeIds = new Set(visibleNodes.map((n) => n.id))
    return Array.from(document.edges.values()).filter(
...
function EditableNode({ node, onMove, onEdit }: EditableNodeProps) {
  return (
    ...
    >
      {node.type === 'page' && <EmbeddedPageEditor pageId={node.contentRef!.id} />}
      {node.type === 'database' && (
      ...
      {node.type === 'table' && <EmbeddedTableView databaseId={node.contentRef!.id} />}
      {node.type === 'text' && <TextEditor content={node.content?.text} />}
      {node.type === 'shape' && (
...
class GraphViewOptimizer {
  ...
  async getVisibleGraph(centerNodeId: string, filters: EdgeFilterConfig): Promise<VisibleGraph> {
