...
- [ ] >80% test coverage on database packages
...
```
@xnet/modules ───> @xnet/database, @xnet/storage
@xnet/workflows ─> @xnet/modules, @xnet/database
...
TriggerType = 'manual' | 'schedule' | 'property_change' | 'record_create' | 'webhook'

// Plugins
PluginId = `plugin:${string}`
PluginPermission = 'read:databases' | 'write:databases' | 'network' | 'notifications'
```
