...
export class FormulaEngine {
  ...
  private tokenize(expression: string): Token[] {
    ...
    while (pos < expression.length) {
      ...
      if (char === '(') { tokens.push({ type: TokenType.LeftParen, value: '(', position: pos++ }); continue; }
      if (char === ')') { tokens.push({ type: TokenType.RightParen, value: ')', position: pos++ }); continue; }
      if (char === ',') { tokens.push({ type: TokenType.Comma, value: ',', position: pos++ }); continue; }
...
export class VectorIndex {
  ...
  static deserialize(data: { vectors: Record<string, number[]>; metadata: Record<string, any> }): VectorIndex {
