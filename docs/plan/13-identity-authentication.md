...
async function unlockIdentity(): Promise<Identity> {
  ...
  const decryptionKey = await crypto.subtle.deriveKey(
    ...
    await crypto.subtle.importKey('raw', assertion.response.signature, 'HKDF', false, ['deriveKey']),
...
interface Identity {
  ...
  encryptionKey: {        // X25519 (encryption)
...
function createIdentity(seed?: Uint8Array): Identity {
  ...
  const encryptionKey = x25519.generateKeyPair(
    hkdf(seed, 'xnet-encryption')
  )
