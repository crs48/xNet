...
xNet apps don't fetch data from a server — they read from a local database (IndexedDB in the browser). This means:
...
The `OfflineQueue` holds up to 1000 entries, persisted to IndexedDB immediately for crash resilience. On reconnect, entries are drained in order. If a broadcast fails, the entry stays at the front of the queue for retry.
...
For forms that create multiple related nodes, use transactions to ensure atomicity:
...
const createProjectWithTasks = async () => {
  ...
  // All three nodes are created atomically in local storage.
  // They sync together when connected, ensuring no dangling references.
}
```

## Storage

xNet uses IndexedDB through the `@xnet/storage` adapter. Data is organized per-node:
