...
When connectivity is restored, the offline queue drains automatically. You don't need to handle this manually — `useNode` and `useQuery` will re-render with the merged state. See the [Offline Patterns](/xNet/guides/offline/) guide for details.
...
See the [Sync Guide](/xNet/guides/sync/) for the full security architecture.
