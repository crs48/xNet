...
| --------- | ------------------ | --------------------------------------------------------------------------------------- |
| `schema`  | `DefinedSchema<P>` | The schema to query. Created with [`defineSchema()`](/xNet/docs/schemas/defineschema/). |
...
Every node includes system fields plus your schema properties:
...
- [`useMutate`](/xNet/docs/hooks/usemutate/) — write data that `useQuery` reads
- [`useNode`](/xNet/docs/hooks/usenode/) — when you need a Yjs document for one node
- [`defineSchema`](/xNet/docs/schemas/defineschema/) — define the schemas you query
