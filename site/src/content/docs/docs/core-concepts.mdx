...
See [defineSchema](/xNet/docs/schemas/defineschema/) and [Property Types](/xNet/docs/schemas/property-types/) for full reference.
...
Every node has system fields added automatically:
...
Your schema properties are merged alongside these fields into a **flat node** — you access `task.title` directly, not `task.properties.title`.
