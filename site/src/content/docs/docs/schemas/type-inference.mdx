...
3. **`FlatNode<S>`** merges the base node fields (`id`, `createdAt`, `updatedAt`, `createdBy`) with the inferred property types to produce the "read" side. This is what you get back from `useQuery` and `useNode`.
