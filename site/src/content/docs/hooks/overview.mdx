...
| ------------------------------------------ | ------------------------------------------------ | ----------------------------------------------------- |
| [`useQuery`](/xNet/hooks/usequery/)   | List or find nodes. Read-only.                   | `{ data, loading, error, reload }`                    |
| [`useMutate`](/xNet/hooks/usemutate/) | Create, update, delete nodes.                    | `{ create, update, remove, restore, mutate }`         |
| [`useNode`](/xNet/hooks/usenode/)     | Edit a single node with collaborative rich text. | `{ data, doc, update, syncStatus, remoteUsers, ... }` |
...
| Hook                                           | Purpose                                    |
| ---------------------------------------------- | ------------------------------------------ |
| [`useIdentity`](/xNet/hooks/useidentity/) | Get the current user's DID and auth status |
