/**
 * Comparison data for /compare (and the landing Landscape section's count).
 *
 * Single source of truth: every table, mobile card, chip, and count on
 * /compare renders from these arrays. When a project ships a status change,
 * edit its row, bump `lastVerified` on its layer, and keep footnotes'
 * sourceUrl pointing at the announcement. Review quarterly. Never state a
 * competitor status claim without a sourced footnote. xNet rows must match
 * site/src/data/roadmap.ts — "shipped" means listed under Built there.
 *
 * Validated at build time by site/scripts/validate-compare.ts.
 */

export type Maturity = 'production' | 'beta' | 'alpha' | 'pre-release' | 'maintenance'

/** A cell is plain text, a ternary ('yes' | 'partial' | 'no'), or a value with a footnote ref. */
export type CellValue = string | { v: string; fn: string }
...
export interface CompareProject {
  ...
  license: string
  bestFor: string
  /** Values for this layer's columns. `license` and `bestFor` resolve from the fields above. */
  dims: Record<string, CellValue>
  /** Extra label → value rows shown in the per-project expansion and mobile cards. */
  details?: Record<string, string>
  /** Row-level footnote refs, rendered as superscripts after the project name. */
  footnotes?: string[]
}
...
export interface CompareLayer {
  ...
  shortTitle: string
  intro: string
  /** How xNet relates to this layer — rendered as a callout under the intro. */
  xnetNote?: string
  lastVerified: string
...
/** Total projects with a full table row across all layers. */
export const rowCount = layers.reduce((n, l) => n + l.projects.length, 0)

/** Total long-tail chip acknowledgements across all layers. */
export const chipCount = layers.reduce((n, l) => n + l.chips.length, 0)
